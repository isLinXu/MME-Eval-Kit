...
#             # 创建标签页
#             with gr.TabItem(f'📊 Perception Leaderboard'):
#                 # 在每个标签页中展示对应列的数据
#                 gr.Markdown(f"## Perception Leaderboard")
#                 sorted_data_frame = data_frame.sort_values('Perception', ascending=False)
#                 gr.components.DataFrame(value=sorted_data_frame, type="pandas", interactive=False, visible=True)
#
#             with gr.TabItem(f'📊 Cognition Leaderboard'):
#                 # 在每个标签页中展示对应列的数据
#                 gr.Markdown(f"## Cognition Leaderboard")
#                 sorted_data_frame = data_frame.sort_values('Cognition', ascending=False)
#                 gr.components.DataFrame(value=sorted_data_frame, type="pandas", interactive=False, visible=True)
#
...
def create_interface():
    with gr.Blocks() as demo:
        ...
        with gr.Tabs(elem_classes='tab-buttons') as tabs:
            ...
            #     gr.components.DataFrame(value=sorted_data_frame, type="pandas", interactive=False, visible=True)
            #
            #     # 在Perception标签页中显示直方图
            #     histogram_path = f"histogram_Perception.png"
            #     plot_histogram(data_frame, 'Perception', output_path=histogram_path)
            #     encoded_histogram_string = save_image_to_base64(histogram_path)
            #     gr.Markdown(f'<center><img src="data:image/png;base64,{encoded_histogram_string}" alt="Perception Histogram" width=640 height=480></center>')
            #
            # with gr.TabItem(f'📊 Cognition Leaderboard'):
            #     # 在每个标签页中展示对应列的数据
            #     gr.Markdown(f"## Cognition Leaderboard")
            ...
            #
            #     # 在Cognition标签页中显示直方图
            #     histogram_path = f"histogram_Cognition.png"
            #     plot_histogram(data_frame, 'Cognition', output_path=histogram_path)
            #     encoded_histogram_string = save_image_to_base64(histogram_path)
            #     gr.Markdown(f'<center><img src="data:image/png;base64,{encoded_histogram_string}" alt="Cognition Histogram" width=640 height=480></center>')
